...
def review_edit(request, pk, review_id):
    ...
    if request.method == "POST":
        ...
        if review_form.is_valid() and review.author == request.user:
            ...
            review.save()
            messages.add_message(
                request, messages.SUCCESS, "Your review has been updated successfully!"
            )
        else:
            messages.add_message(
                request,
...
def review_delete(request, pk, review_id):
    ...
    if review.author == request.user:
        review.delete()
        messages.add_message(
            request, messages.SUCCESS, "Your review has been successfully deleted!"
        )
    else:
        messages.add_message(
            request, messages.ERROR, "There was an error trying to delete your review"
        )
